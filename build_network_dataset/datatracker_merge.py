...
from rfc_decimal_verify import (
...
def merge_datatracker_rfcs_into_by_proto(
    ...
    *,
    ...
    # Only query Datatracker when IANA Reference produced no RFC for this seed; otherwise keep Reference-only sets.
    iana_ref_has_rfc = {sid for sid in seed_ids if by_proto.get(sid)}
    seed_ids_dt = seed_ids - iana_ref_has_rfc
    n_skip_iana_ref = len(iana_ref_has_rfc)
    if not seed_ids_dt:
        print(
            f"Network Datatracker: skipped for all {n_skip_iana_ref} seed(s) "
            "(IANA Reference already lists at least one RFC per seed).",
            file=sys.stderr,
            flush=True,
        )
        return
    ...
    skipped_short = sum(1 for sid in seed_ids_dt if not phrases_by_key.get(sid, ()))
    reuse = len(seed_ids_dt) - skipped_short - pending
    print(
    ...
    if pending:
        ...
        if total_http > 0:
            http_progress = DatatrackerHttpProgress(
                total_http, desc="Network Datatracker HTTP"
            )
            http_progress.start()
        done_n = 0
        try:
