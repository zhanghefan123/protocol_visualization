...
```powershell
pip install -r build_dataset\requirements.txt
```

> **说明**：单独跑 `python -m rfc_fetch` 时**可以不装**这些包（rfc_fetch 只用标准库）。要跑完整流水线或画图，需要执行上面这行。
...
| `output/rfc_fetch/rfc_txt/` | RFC 正文 `.txt` 缓存（用于默认开启的「正文里是否提到端口」校验） |
...
| `output/cache/rfc_editor/` | RFC Editor 索引 `rfc-index.xml` 与各 RFC 参考文献缓存（体积会变大，属正常） |
...
├── rfc_fetch/                # IANA + Datatracker → CSV（标准库）
├── build_dataset/            # 应用层 seeds + rfc_editor_graph.py
├── build_network_dataset/    # 网络层 seeds 生成脚本
├── viz/                      # 时间线图渲染
...
2. **应用层种子**  
   `python build_dataset/generate_seeds.py`  
   → `output/app_graph/protocol_seeds.yaml`

3. **应用层图**  
   `python build_dataset/rfc_editor_graph.py --workers 16`  
   → `output/app_graph/nodes.csv`、`edges.csv`  
   （默认 `--cache` 为 `output/cache/rfc_editor/`）
...
5. **网络层图**  
   `python build_dataset/rfc_editor_graph.py --seeds output/network_graph/protocol_seeds_network.yaml --out output/network_graph --workers 16`  
...
- **仅标准库**：不必 `pip install` 即可运行 `python -m rfc_fetch`。
...
A：端口表是「每端口/传输一行」；种子按 **协议名** 合并，且无名服务不进 `generate_seeds.py` 的协议表。详见此前说明或代码注释。
