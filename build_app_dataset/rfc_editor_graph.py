...
Default ``--cache`` is ``output/app_graph/cache/`` (see ``project_paths.OUTPUT_CACHE_RFC_EDITOR``).
RFC plaintext/XML for reference extraction uses ``--rfc-body-cache`` (shared with ``rfc_fetch``; see ``project_paths.RFC_BODY_CACHE_DIR``).

Install deps: pip install -r build_app_dataset/requirements.txt
"""
...
from timeline_data import enrich_nodes_list_descriptions
...
from rfc_fetch.rfc_body_port import fetch_rfc_plaintext_cached
...
def fetch_rfc_reference_numbers(
    rfc_num: int,
    *,
    rfc_body_cache_dir: Path,
    fetch_timeout_s: float,
) -> Tuple[Set[int], str]:
    """
    Parse RFC plaintext for ``RFCxxxx`` mentions, biased toward references sections.
    Uses on-disk cache under ``rfc_body_cache_dir`` before HTTP.
    Returns: (referenced_rfc_numbers, detail_source)
    """
    txt = fetch_rfc_plaintext_cached(
        rfc_num, cache_dir=rfc_body_cache_dir, timeout=fetch_timeout_s
    )
...
def cached_rfc_reference_numbers(
    graph_refs_cache_dir: str,
    rfc_num: int,
    *,
    rfc_body_cache_dir: Path,
    fetch_timeout_s: float,
    ...
    """
    cache_ref_path = os.path.join(graph_refs_cache_dir, f"rfc{rfc_num}.refs.txt")
    ...
    refs, detail_src = fetch_rfc_reference_numbers(
        rfc_num,
        rfc_body_cache_dir=rfc_body_cache_dir,
...
def main() -> int:
    ...
    ap.add_argument(
        ...
        help=f"Cache directory (rfc-index.xml + per-RFC reference cache; default: {OUTPUT_CACHE_RFC_EDITOR})",
    )
    ap.add_argument(
        "--rfc-body-cache",
        type=Path,
        default=RFC_BODY_CACHE_DIR,
        help=f"Directory for cached RFC .txt bodies (reference extraction; default: {RFC_BODY_CACHE_DIR})",
    )
    ap.add_argument(
        "--rfc-fetch-timeout",
        type=float,
        default=60.0,
        help="HTTP timeout (seconds) when fetching RFC plaintext/XML into --rfc-body-cache",
    )
    ...
    args = ap.parse_args()
    rfc_body_cache = Path(args.rfc_body_cache)
    fetch_timeout = float(args.rfc_fetch_timeout)

    if not os.path.isfile(args.seeds):
        print(f"Seeds file not found: {args.seeds}", file=sys.stderr)
        print("Generate it with: python build_app_dataset/generate_app_seeds.py", file=sys.stderr)
        return 2
    ...
    rfc_body_cache.mkdir(parents=True, exist_ok=True)
    ...
    if expand_enabled:
        ...
        while q and len(discovered) < max_total:
            ...
            refs, detail_src = cached_rfc_reference_numbers(
                args.cache,
                rfc,
                rfc_body_cache_dir=rfc_body_cache,
    ...
    enrich_nodes_list_descriptions(
        nodes,
        ports_csv=IANA_PORTS_CSV,
        protocol_csv=NETWORK_PROTOCOL_NUMBERS_CSV,
    )
    for row in nodes:
    ...
    with open(nodes_path, "w", newline="", encoding="utf-8") as f:
        w = csv.DictWriter(
            f,
            fieldnames=["id", "label", "birth_date", "defining_rfcs", "description", "source"],
        )
        w.writeheader()
